...
function ca.SOS_voice_overwrite(target,state)
  ...
  if state then
    ...
    local l_in = params:get("SOS_L_in_"..target.bank_id) == 2 and 1 or 0
    local r_in = params:get("SOS_R_in_"..target.bank_id) == 2 and 1 or 0
...
function ca.sample_to_pad_callback(path,i,p)
  
  if path ~= "cancel" and path ~= "" then
    ca.sample_to_pad(path,i,p)
  end
