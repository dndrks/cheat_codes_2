...
function f_m.draw_menu()
  ...
  elseif _fm_.menu_layer == 3 then
    ...
    elseif _fm_.main_sel == 3 then
      ...
      screen.text("#")
      screen.move(80,6)
      screen.text_center("BEATS")
      screen.move(110,6)
      screen.text_center("SLOT")
      screen.level(3)
      local sel_x = 74+(grid_actions.index_to_grid_pos(_fm_.song_col[_fm_.bank_sel],2)[1]-1)*30
      ...
      for i = 1+(10*page), 10+(10*page) do
        screen.move(80+(grid_actions.index_to_grid_pos(util.wrap(i,1,10),2)[1]-1)*30,10+(10*grid_actions.index_to_grid_pos(util.wrap(i,1,10),2)[2]))
        screen.level((_fm_.song_col[_fm_.bank_sel] == grid_actions.index_to_grid_pos(i,2)[1] and _fm_.song_line[_fm_.bank_sel] == grid_actions.index_to_grid_pos(i,2)[2]) and 0 or 15)
        if grid_actions.index_to_grid_pos(util.wrap(i,1,10),2)[1] == 1 and grid_actions.index_to_grid_pos(i,2)[2] <= song_atoms.bank[bank_id].end_point then
          screen.text_center(song_atoms.bank[bank_id].lane[grid_actions.index_to_grid_pos(i,2)[2]].beats)
          screen.level(15)
          screen.move(60+(grid_actions.index_to_grid_pos(util.wrap(i,1,10),2)[1]-1)*30,10+(10*grid_actions.index_to_grid_pos(util.wrap(i,1,10),2)[2]))
          screen.text(grid_actions.index_to_grid_pos(i,2)[2])
        elseif grid_actions.index_to_grid_pos(i,2)[2] <= song_atoms.bank[bank_id].end_point then
          local target = song_atoms.bank[bank_id].lane[grid_actions.index_to_grid_pos(i,2)[2]][pattern_names[grid_actions.index_to_grid_pos(i,2)[1]-1]].target
          if target > 0 then
...
function f_m.process_encoder(n,d)
  ...
  elseif _fm_.menu_layer == 3 then
    ...
    elseif _fm_.main_sel == 3 then
      ...
      elseif n == 2 then
        if _fm_.song_col[_fm_.bank_sel] == 2 then
          if d > 0 then
            ...
            _fm_.song_col[_fm_.bank_sel] = util.clamp(_fm_.song_col[_fm_.bank_sel] + d,1,2)
          end  
        else
          if d < 0 then
            local current_line = _fm_.song_line[_fm_.bank_sel]
        ...
        --   _fm_.song_col[_fm_.bank_sel] = 1
        --   _fm_.song_line[_fm_.bank_sel] = util.clamp(_fm_.song_line[_fm_.bank_sel] + (1*d),1,song_atoms.bank[_fm_.bank_sel].end_point)
        -- end
      elseif n == 3 then
