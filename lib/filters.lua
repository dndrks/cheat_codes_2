...
function filters.init()
  ...
  for i = 1,3 do
    ...
    for j = 1,#filter_types do
      ...
      params:add_binary("filter "..filter_types[j].." active "..i, "filter "..banks[i].." "..filter_types[j].." active","toggle",0)
      params:set_action("filter "..filter_types[j].." active "..i, function(x)
        if x == 1 then
      ...
      filter[i].freq.attack_clock = clock.run(
        function()
          ...
          while compare_freq_values(i,min_val,max_val,"attack") do
            ...
            -- iterate_freq_values(i,filter[i].freq.min,filter[i].freq.max,delta,"attack")
